...
const DELAY_REASONS = [
  { de: "aufgrund einer Störung im Betriebsablauf", en: "due to an operational disruption" },
  ...
  { de: "wegen des Fahrgastwechsels",               en: "due to extended passenger exchange" },
];
...
function randomDelay() {
  ...
  if (r < 0.90) return Math.floor(Math.random() * 20) + 33; // 33-52: philosophical
...
function generateAnnouncement() {
  ...
  if (r < 0.55) {
    ...
    data = { line: line.id, dest: line.dest, platform: line.platform, mins, reason };
  } else if (r < 0.70) {
    type = 'platformChange';
    const newPlatform = String(Math.floor(Math.random() * 4) + 1);
    const oldPlatform = line.platform !== newPlatform ? line.platform :
  ...
  for (const lang of Object.keys(T[type])) {
    try {
      if (type === 'delay')          translations[lang] = T[type][lang](data.line, data.dest, data.platform, data.mins, data.reason);
      else if (type === 'platformChange') translations[lang] = T[type][lang](data.line, data.dest, data.oldPlatform, data.newPlatform);
      else if (type === 'cancelled') translations[lang] = T[type][lang](data.line, data.dest, data.platform);
      else if (type === 'arriving')  translations[lang] = T[type][lang](data.line, data.dest, data.platform);
    } catch(e) { translations[lang] = translations['en']; }
  }
  ...
  return {
    ...
    platform: type === 'platformChange' ? data.newPlatform : data.platform || line.platform,
  };
}

// Export for use in main app
if (typeof module !== 'undefined') module.exports = { generateAnnouncement, LINES, T };
